...
load('NatalyaElecMap.mat')

[~,idx] = sort(map(:,2),'ascend');
disp(map(idx,:))
rMap = map(idx,1);
neuralDataDS = neuralDSData;
for blk = 1:H.noOfCuts
    neuralDataDS{blk} = neuralDSData{blk}(:,rMap);
end
load([data_dir filesep 'trft_normMean_' task_dir '.mat'],'trft_normMean')
trft_normMean = trft_normMean(:,rMap);

ch2rm = [16 55 56 63 64];
realCh2rm = rMap(ch2rm)
chUsed = setdiff(1:64,realCh2rm);

extentionShift = 250;    % pts
flexionShift = 340;     % pts
...
% for blk = 1:H.noOfCuts
%     neuralDataDS{blk} = neuralDSData{blk}-repmat(mean(neuralDSData{blk}(:,chUsed),2),1,64);
% end
